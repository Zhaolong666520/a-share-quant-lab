...
- 英文回测指标词汇表，逐项说明定义、解读方式、公式口径与主要限制。
...
- `paper-init`、`paper-run`、`paper-status`、`paper-daily` 命令，以及 Windows 一键运行、计划任务安装和卸载入口。
- 可撤销的 Windows 工作日计划任务：每日数据更新、健康检查与模拟盘运行，并保存成功/失败状态、日志和本地失败通知。
- 确定性 HTML/JSON/CSV/PNG 审计报告，报告中的费用和滑点均明确为假设情景。

### Changed

- 本地锁文件、自动化日志和不可变运行归档默认不进入 Git，降低误提交个人模拟记录的风险。

### Security

- 模拟盘只读取通过清单、更新摘要、SHA-256 与陈旧度检查的本地行情；数据异常时不产生订单或状态变更。
- 不连接券商、不保存凭证、不发送真实订单；报告失败不会回滚已提交账本。
- Windows 计划任务安装器拒绝使用 `.worktrees` 临时目录，并限制为当前交互用户和普通权限。

### Fixed

- 跨进程文件锁测试固定使用 UTF-8 输入输出，避免 GitHub Windows runner 的系统代码页把正确的锁冲突误报为测试失败。
